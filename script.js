function getWeatherFunction(clickedAnchor){
    ...
    userInputForm.addEventListener("submit", (event)=>{
        ...
        /* if click on the form, do something. if click on any anchor, do something. */
        if (clickedAnchor) {
            weatherAPI = clickedAnchor;
        }
...
function clickedPreviousFunction() {
    document.querySelector(".history a").addEventListener("click", (event)=>{
        event.preventDefault();
        let clickedAnchor = event.target.href;
